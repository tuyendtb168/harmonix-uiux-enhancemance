...
import { VaultDetailPanel3 } from './VaultDetailPanel3'
...
function VaultListCard({ vault, isSelected, onSelect, hasPending, hasCompleted }: { vault: Vault; isSelected: boolean; onSelect: () => void; hasPending?: boolean; hasCompleted?: boolean }) {
  ...
  return (
    <button onClick={onSelect} className="w-full text-left rounded-xl p-3.5 transition-all"
      ...
      {(isRec || isNew || hasPending || hasCompleted) && (
      ...
      <div className="flex items-start gap-3">
        <div style={{ position: 'relative', flexShrink: 0 }}>
          ...
          {hasCompleted && !hasPending && (
            <span style={{ position: 'absolute', top: -3, right: -3, width: 10, height: 10, borderRadius: '50%', background: '#22c55e', border: '2px solid #fff', display: 'block' }} />
          )}
        </div>
...
function EarnPanel({ vaults, selectedId, onSelect, pendingVaultIds, completedVaultIds }: { vaults: Vault[]; selectedId: string | null; onSelect: (v: Vault) => void; pendingVaultIds?: Set<string>; completedVaultIds?: Set<string> }) {
  ...
  return (
    <div className="flex flex-col gap-2.5">
      ...
      <div className="flex flex-col gap-2">
        {shown.map(v => <VaultListCard key={v.id} vault={v} isSelected={selectedId === v.id} onSelect={() => onSelect(v)} hasPending={pendingVaultIds?.has(v.id)} hasCompleted={completedVaultIds?.has(v.id)} />)}
      </div>
    </div>
  )
}
...
// ── Mock pre-seeded state ─────────────────────────────────────────────────────

const MOCK_POSITIONS: Record<string, { value: number; valueFormatted: string; pnl: number; pnlFormatted: string; pnlPercent: number; depositedAt: string; pendingWithdrawal?: number; pendingWithdrawalFormatted?: string; pendingArrival?: string; completedWithdrawal?: number; completedWithdrawalFormatted?: string; completedAt?: string; completedTxHash?: string }> = {
  'delta-neutral-usdc': {
    value: 8450,
    ...
    pendingWithdrawalFormatted: '$2,500.00',
    pendingArrival: 'Jun 30–Jul 2, 2026',
  },
  'eth-yield-max': {
    value: 5200,
    valueFormatted: '$5,200.00',
    pnl: 187,
    pnlFormatted: '+$187.00',
    pnlPercent: 3.73,
    depositedAt: 'Jan 8, 2025',
    completedWithdrawal: 3200,
    completedWithdrawalFormatted: '$3,200.00',
    completedAt: 'Jun 25, 2026',
    completedTxHash: '0xa3f8d2c1b5e94f7a6d0e321c8b7a5f2d9e4c1b0a',
  },
}

const MOCK_PENDING_IDS = new Set(['delta-neutral-usdc'])
const MOCK_COMPLETED_IDS = new Set(['eth-yield-max'])

// ── Page ──────────────────────────────────────────────────────────────────────

export function Earn3Page() {
  ...
  const [pendingVaultIds, setPendingVaultIds] = useState<Set<string>>(MOCK_PENDING_IDS)
  const [completedVaultIds] = useState<Set<string>>(MOCK_COMPLETED_IDS)
  ...
  const handleWithdrawSuccess = (amount: number) => {
    ...
    const arrival = (() => {
      ...
      return `${d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}–${d2.toLocaleDateString('en-US', { day: 'numeric', year: 'numeric' })}`
    })()
        ...
        <div className="flex gap-5 items-start">
          <div className="w-80 shrink-0 sticky top-[80px] self-start">
            {isLoading ? (
              ...
              <EarnPanel vaults={vaults} selectedId={selectedVault?.id ?? null} onSelect={setSelectedVault} pendingVaultIds={pendingVaultIds} completedVaultIds={completedVaultIds} />
            )}
          </div>
          <div className="flex-1 min-w-0">
            {isLoading ? (
