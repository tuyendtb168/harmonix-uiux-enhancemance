...
import { X, Star, Sparkles, ShieldCheck, Download, Zap, Shield, Activity, ArrowDownLeft, ArrowUpRight } from 'lucide-react'
...
// ── Overview tab body ─────────────────────────────────────────────────────────
function OverviewBody({ vault, onDeposit, onSwitchToComposition }: { vault: Vault; onDeposit: (id: string) => void; onSwitchToComposition: () => void }) {
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-2xl p-5 flex flex-col gap-4" style={{ background: '#fff', border: '1px solid #e2e8f0' }}>
          ...
          <div className="rounded-xl p-4" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
            ...
            <p className="text-4xl font-black tabular-nums leading-none" style={{ color: '#0f172a' }}>$12,450.23</p>
            <p className="text-sm font-semibold mt-1.5" style={{ color: '#16a34a' }}>+$1,230.23 <span style={{ color: '#94a3b8' }}>(10.00%)</span></p>
          </div>
          {/* Stats grid */}
          <div className="grid grid-cols-2 gap-3">
            {[
              ['Net deposited',    '$11,220.00'],
              ['Total earnings',   '$1,230.23'],
              ['Shares owned',     '12,320.45 ha' + vault.asset],
              ['Pending withdraw', '$0.00'],
            ].map(([l, v]) => {
              const isPending = l === 'Pending withdraw'
              const isHighlit = isPending && v !== '$0.00'
              return (
...
interface VaultDetailPanel2Props {
  vault: Vault
  onClose: () => void
  onDeposit: (id: string) => void
}

export function VaultDetailPanel2({ vault, onClose, onDeposit }: VaultDetailPanel2Props) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Tab content */}
      {tab === 'overview'     && <OverviewBody vault={vault} onDeposit={onDeposit} onSwitchToComposition={() => setTab('composition')} />}
